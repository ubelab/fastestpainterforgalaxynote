package com.invenktion.android.fastestpainter.sgnote.view;
...
import com.invenktion.android.fastestpainter.sgnote.core.AmmoManager;
...
import com.invenktion.android.fastestpainter.sgnote.utils.ColorUtils;
import com.invenktion.android.fastestpainter.sgnote.utils.FilterUtils;
...
public class FingerPaintDrawableView extends View implements OnTouchListener{
    ...
    private void touch_move(float x, float y,boolean PINCH_ZOOM) {
        ...
        if (dx >= TOUCH_TOLERANCE || dy >= TOUCH_TOLERANCE) {
            ...
            //quando finisce il tempo e il dito è ancora giu
	...
	*/
    ...
    public void startResultElaboration() {
        ...
        Thread t = new Thread() {
            public void run() {
            	    try {//necessario poichè a volte le bitmap vengono reciclate e questo thread ci lavora sopra
            		//Elaboro
	     	        if(contour != null && colored != null) {
	     	        	if(!resultComputed) {//Per non fare questo calcolo oneroso più di una volta
        	    	...
        	    	//Ok siamo nel caso in cui cerchiamo di usare una bitmap che abbiamo già reciclato
					e.printStackTrace();
				}
            }
        };
